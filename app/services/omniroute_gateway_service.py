...
def _canonicalize_reported_model(*, provider: str, requested_model: str, reported_model: str) -> tuple[str, tuple[str, ...]]:
    """Accept only documented deterministic provider aliases, never a different model.

    OmniRoute/OpenCode currently reports ``big-pickle`` after an explicit
    ``oc/big-pickle`` request. The provider namespace is transport metadata,
    not a model-selection decision. Any other mismatch still fails closed.
    """
    if reported_model == requested_model:
        return requested_model, ()
    if (
        normalize_provider_id(provider) == "opencode"
        and requested_model.startswith("oc/")
        and reported_model == requested_model.removeprefix("oc/")
    ):
        return requested_model, (f"provider_reported_model_alias:{reported_model}",)
    raise OmniRouteGatewayIntegrityError(
        f"OmniRoute model mismatch expected={requested_model!r} reported={reported_model!r}"
    )
    ...
    def execute(
        ...
        *,
        ...
        self.artifact_service.download(
            repository=self.repository,
            run_id=dispatched.run_id,
            artifact_name=OMNIROUTE_ARTIFACT_NAME,
            output_dir=output_dir,
        )
        files = sorted(output_dir.rglob("result.json"))
        if len(files) != 1:
            raise OmniRouteGatewayError(
                f"Expected exactly one OmniRoute result.json, found {len(files)}"
            )
        try:
        ...
        canonical_model, alias_warnings = _canonicalize_reported_model(
            provider=str(payload["provider"]),
            requested_model=model,
            reported_model=str(payload["model"]),
        )
        warnings = tuple(payload.get("warnings") or ()) + alias_warnings
        return OmniRouteTransportResult(
            status=str(payload["status"]),
            provider=str(payload["provider"]),
            model=canonical_model,
            text=str(payload["text"]),
            omniroute_version=str(payload["omniroute_version"]),
            runtime_identity=str(payload["runtime_identity"]),
            execution_ref=f"github-actions:{dispatched.run_id}",
            elapsed_seconds=float(
                payload.get("elapsed_seconds") or (time.monotonic() - started)
            ),
            fallback_occurred=bool(payload["fallback_occurred"]),
...
def _provider_record(provider: str, registry: GlobalCapabilityRegistry):
    ...
    matches = [
        ...
        and r.provider_id
        and normalize_provider_id(r.provider_id) == normalized
    ]
    if len(matches) != 1:
        raise PermissionError(
            "OmniRoute provider Registry metadata is missing or ambiguous"
        )
    return matches[0]


def execute_omniroute_gateway(
    *,
    prompt: str,
    routing_decision: HarnessRoutingDecision,
    authorization: HarnessAuthorization | dict[str, Any] | str,
    transport: OmniRouteExecutionTransport,
    registry: GlobalCapabilityRegistry = GLOBAL_CAPABILITY_REGISTRY,
    quota_available: bool | None = True,
) -> OmniRouteGatewayEvidence:
    ...
    assessment = enforce_zero_cost(
        provider_record.cost_class,
    ...
    resolved = validate_harness_authorization(
        ...
        expected_subject=(
            f"provider:{normalize_provider_id(routing_decision.selected_provider)}"
        ),
    )
    result = transport.execute(
        prompt=prompt,
        provider=normalize_provider_id(routing_decision.selected_provider),
        model=routing_decision.selected_model,
        execution_id=resolved.execution_id,
    )
    if result.status != "EXECUTED":
        raise OmniRouteGatewayError("OmniRoute execution did not succeed")
    if normalize_provider_id(result.provider) != normalize_provider_id(
    ...
    if result.fallback_occurred:
        raise OmniRouteGatewayIntegrityError(
            "OmniRoute reported unauthorized fallback"
        )
    return OmniRouteGatewayEvidence(
        ...
        execution_id=resolved.execution_id,
        capability=routing_decision.selected_capability_id,
        provider=normalize_provider_id(routing_decision.selected_provider),
        model=routing_decision.selected_model,
        executor=OMNIROUTE_EXECUTOR_BINDING,
        omniroute_version=result.omniroute_version,
