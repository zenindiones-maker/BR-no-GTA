...
def build_semantic_planner_prompt(
    ...
    *,
    ...
    instructions = (
        ...
        "R=research/evidence/retrieval/fact-check; E=editorial/script/content/"
        "YouTube script review/content production planning; D=software/repository/"
        "system engineering plus provider/runtime incident diagnosis, failure "
        "classification, root-cause and recovery review; "
        "X=runtime/render/narration/media/delivery; "
        "C=decision/advisory. Never use D as a generic synonym for doing work. "
        "If caps contains an ID, it must come from capabilities, act must be "
        "literally present in that "
        "capability's actions, every operation the task actually needs must be "
        "present in that capability's operations, and risk/side effects must not "
        "exceed that capability's side_effect_class. Never assign semantic reasoning "
        "to deterministic collection, retrieval, or fact-check executors. If a goal "
        "needs fresh evidence plus interpretation, use a deterministic evidence task "
        "(task class evidence-collection/fresh-evidence-collection/knowledge-retrieval/"
        "fact-check as appropriate) followed by a dependency-linked semantic task. "
        "telegram.input.ingest is ingress only (human->Telegram->Harness) and must never "
        "be selected for outbound human delivery; outbound work requires a telegram-outbound "
        "capability or a deferred infrastructure delivery step. A candidate's "
        "output contract must directly satisfy the task expected output: review-only "
        "capabilities do not generate a script, and production-plan capabilities do "
        "not generate editorial copy. "
