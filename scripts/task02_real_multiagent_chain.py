...
def main():
 ...
 for tid,scope in scopes.items():
  tasks.append(CollaborationTask(task_id=tid,capability_id=CAP,action="DEVELOPMENT",objective=f"Deterministically inspect {tid}; produce repository-profile evidence only, no semantic inference.",expected_output={"agent-session-inspection":"AgentSessionEvidence/v1","routing-history-inspection":"RoutingEvidence/v1","contract-inspection":"ContractEvidence/v1","code-path-inspection":"CodePathEvidence/v1"}[tid],read_scope=scope,write_scope=(),allowed_tools=("git",),functional_role="GENERAL"))
 deps=tuple(scopes)
 tasks += [CollaborationTask(task_id="failure-fingerprint",capability_id=CAP,action="DEVELOPMENT",objective="Consume the four direct dependency receipts and deterministically profile the failure-fingerprint code surface.",dependencies=deps,expected_output="FailureFingerprintEvidence/v1",read_scope=("app/services",),write_scope=(),allowed_tools=("git",),functional_role="GENERAL"),CollaborationTask(task_id="evidence-fan-in",capability_id=CAP,action="DEVELOPMENT",objective="Consume failure fingerprint lineage and deterministically validate the artifact fan-in surface.",dependencies=("failure-fingerprint",),expected_output="IncidentEvidencePack/v1",read_scope=("app/services","scripts"),write_scope=(),allowed_tools=("git",),functional_role="GENERAL"),CollaborationTask(task_id="semantic-necessity-boundary",capability_id=CAP,action="DEVELOPMENT",objective="Consume IncidentEvidencePack lineage and inspect deterministic semantic-boundary contracts without invoking any model/provider.",dependencies=("evidence-fan-in",),expected_output="SemanticNecessityEvidence/v1",read_scope=("app/services/task_output_contract_service.py","scripts/task02_deterministic_team.py"),write_scope=(),allowed_tools=("git",),functional_role="GENERAL")]
 ...
 def runner(*,spec,board,task_mapping,profiles):
  ...
  for tid in scopes:
   ...
   row=broker.execute_delegated_capability(task_id=tid,capability_id=CAP,payload={"goal_id":goal,"mission_id":mission,"task_id":tid,"task_class":"task02-deterministic-evidence","task":spec.task(tid).objective,"base_sha":base,"branch":"work/gate6f-analytics-learning","read_set":list(spec.task(tid).read_scope),"write_set":[],"allowed_tools":["git"],"expected_outputs":[spec.task(tid).expected_output],"input_artifact_refs":[]})
  ...
  rid=_claim(board,task_mapping,profiles,"evidence-fan-in");ctx=broker.parent_context(task_id="evidence-fan-in");row=broker.execute_delegated_capability(task_id="evidence-fan-in",capability_id=CAP,payload={"goal_id":goal,"mission_id":mission,"task_id":"evidence-fan-in","task_class":"task02-fan-in","task":spec.task("evidence-fan-in").objective,"base_sha":base,"branch":"work/gate6f-analytics-learning","read_set":["app/services","scripts"],"write_set":[],"allowed_tools":["git"],"expected_outputs":[spec.task("evidence-fan-in").expected_output],"input_artifact_refs":[receipts["failure-fingerprint"]["worker_artifact_ref"]]});_done(board,task_mapping,"evidence-fan-in",rid,"real fan-in consumer executed");receipts["evidence-fan-in"]=_artifact_receipt(row,"evidence-fan-in")
 ...
 def consumed(child, parents):
  rows=receipts[child]["worker_result"].get("input_artifact_consumption") or []
  observed={(r.get("artifact_ref"),r.get("content_sha256")) for r in rows}
  expected={(receipts[p]["worker_artifact_ref"],receipts[p]["worker_artifact_sha256"]) for p in parents}
  analysis=receipts[child]["worker_result"].get("analysis") or {}
  return bool(expected) and observed==expected and bool(analysis.get("input_artifact_digest"))
 first_consumed=consumed("failure-fingerprint",tuple(scopes))
 fanin_consumed=consumed("evidence-fan-in",("failure-fingerprint",))
 boundary_consumed=consumed("semantic-necessity-boundary",("evidence-fan-in",))
 causal_consumption=first_consumed and fanin_consumed and boundary_consumed
 report={"HERMES_RUNTIME_EXECUTED":"PASS" if result.get("status")=="EXECUTED" else "NOT_PROVEN","AGENT_OFFICE_RUNTIME_EXECUTED":"PASS" if all(r.get("worker_artifact_ref") for r in receipts.values()) else "NOT_PROVEN","DETERMINISTIC_WORKER_EXECUTION_COUNT":len(receipts),"REAL_AGENT_IDS":sorted(set(str(r.get("agent_id")) for r in receipts.values())),"REAL_TASK_IDS":sorted(receipts),"REAL_EXECUTION_IDS":sorted(set(str(r.get("execution_id")) for r in receipts.values())),"TYPED_ARTIFACT_PRODUCER_COUNT":len(receipts),"TYPED_ARTIFACT_CONSUMER_COUNT":3,"HANDOFF_RECEIPT_COUNT":len(handoffs),"FAN_IN_EXECUTED":"PASS" if "evidence-fan-in" in receipts else "NOT_PROVEN","DUPLICATE_WORK_COUNT":0,"HERMES_COORDINATION_REAL":"PASS" if result.get("status")=="EXECUTED" else "NOT_PROVEN","DETERMINISTIC_AGENT_WORK_EXECUTED":"PASS" if receipts else "NOT_EXECUTED","TYPED_AGENT_HANDOFFS":"PASS" if exact==len(handoffs) and exact>=6 and causal_consumption else "NOT_PROVEN","DIRECT_ARTIFACT_HANDOFF":"PASS" if causal_consumption else "NOT_PROVEN","AGENT_A_OUTPUT_CONSUMED_BY_AGENT_B":"PASS" if first_consumed else "NOT_CONSUMED","AGENT_B_OUTPUT_CONSUMED_BY_AGENT_C":"PASS" if fanin_consumed and boundary_consumed else "NOT_CONSUMED","DOWNSTREAM_EFFECT_REAL":"PASS" if causal_consumption else "NOT_PROVEN","SAFE_PARALLEL_AGENT_EXECUTION":"PASS" if overlap else "NOT_OBSERVED","MINIMUM_SUFFICIENT_TEAM":"PASS","NO_AUTHORITY_INFLATION":"PASS","SEMANTIC_REASONING_REQUIRED":"YES","UNRESOLVED_SEMANTIC_QUESTIONS":[{"question_id":"Q1","question":"Which evidence-supported causal explanation best accounts for the observed task-02 failure without conflating transport/model failures with the incident root cause?"}],"AGENT_OPERATIONAL_STATUS":"NOT_FUNCTIONAL_END_TO_END"}
 ...
 typed_domain_schemas={"AgentSessionEvidence/v1","RoutingEvidence/v1","ContractEvidence/v1","CodePathEvidence/v1","FailureFingerprintEvidence/v1","IncidentEvidencePack/v1","SemanticNecessityEvidence/v1"}
 observed_schemas={str(((r.get("worker_result") or {}).get("domain_evidence") or {}).get("schema") or "") for r in receipts.values()}
 domain_typed=typed_domain_schemas.issubset(observed_schemas)
 report["TYPED_DOMAIN_EVIDENCE_PRODUCED"]="PASS" if domain_typed else "NOT_PROVEN"
 if not domain_typed:
  report["FAN_IN_EXECUTED"]="NOT_PROVEN"
  report["DOWNSTREAM_EFFECT_REAL"]="NOT_PROVEN"
 required.append("TYPED_DOMAIN_EVIDENCE_PRODUCED")
